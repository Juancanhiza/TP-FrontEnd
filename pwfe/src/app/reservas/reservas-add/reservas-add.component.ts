...
export class ReservasAddComponent implements OnInit, AfterViewInit {
  ...
  filter = () => {
    var m = null;
    if($('#autocomplete-input-medicos').val() in this.medicosMap && $('#fecha').val() != ''){
      m = Number(this.medicosMap[$('#autocomplete-input-medicos').val()]);
      this.saveData.fechaCadena = $('#fecha').val().replace('/','').replace('/','');
      this.saveData.idEmpleado = Number(this.medicosMap[$('#autocomplete-input-medicos').val()]);
      this.api.filterAdd(m, $('#fecha').val().replace('/','').replace('/',''), $('#soloLibres').is(":checked")).subscribe(
        data => {
          this.table = true;
          this.data = data;
  ...
  post = () => {
    ...
    if($('#autocomplete-input-clientes').val() in this.clientesMap   ){
      ...
      console.log(e);
      this.api.postReserva(e).subscribe(
        data => {
